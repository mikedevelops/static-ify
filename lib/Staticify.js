class Staticify {
    ...
    async start () {
        ...
        const html = await this.requestService.get(this.options.request);

        // start parsing for assets
        const parsedHtml = this.replaceService.replace(this.options.request, this.options.target, html);

        // add HTML to resource manager
        this.resourceManager.addHtml(parsedHtml);
